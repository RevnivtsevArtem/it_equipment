...
def fit_model_and_evaluate(
    ...
    y_train = df_train[target_column].fillna(0).astype(int).values

    X_test = df_test[categorical_cols + numeric_cols]
    y_test = df_test[target_column].fillna(0).astype(int).values
...
def fit_on_full_and_save(
    ...
    y = df[target_column].fillna(0).astype(int).values
