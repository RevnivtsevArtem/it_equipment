...
def _get_available_models(models_dir: str = "models") -> list[str]:
...
def page_prediction(df: pd.DataFrame, cfg: dict) -> None:
    """Прогноз вероятности замены устройства по всем обученным моделям."""
    st.subheader("Прогноз необходимости замены устройства")

    available_models = _get_available_models()

    if not available_models:
        st.warning("В папке models не найдено сохранённых моделей. Сначала обучите модель.")
        return

    threshold = st.slider("Порог принятия решения (threshold)", 0.1, 0.9, 0.5, 0.05)
    ...
    if st.button("Рассчитать вероятности по всем моделям"):
        ...
        for model_file in available_models:
            ...
            st.markdown(f"### Модель: {model_file}")
            st.progress(probability)

            decision = 1 if probability >= threshold else 0

            if decision == 1:
                st.error(f"Вероятность {percent_value:.2f}% ≥ threshold {threshold:.2f} → Требуется замена")
            else:
                st.success(f"Вероятность {percent_value:.2f}% < threshold {threshold:.2f} → Замена не требуется")
