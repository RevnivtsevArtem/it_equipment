...
def _apply_ui_style() -> None:
    """Небольшая стилизация, чтобы sidebar и отступы выглядели стабильно и 'как на скринах'."""
    st.markdown(
        """
        <style>
          /* Немного расширяем sidebar на desktop */
          section[data-testid="stSidebar"] { width: 340px; }
          section[data-testid="stSidebar"] > div { width: 340px; }

          /* Заголовки в sidebar */
          .sb-title { font-size: 20px; font-weight: 700; margin: 6px 0 2px 0; }
          .sb-sub   { color: rgba(49,51,63,0.7); font-size: 13px; margin: 0 0 10px 0; }
          .sb-sep   { margin: 10px 0 8px 0; border-bottom: 1px solid rgba(49,51,63,0.15); }

          /* Чуть компактнее radio */
...
def page_prediction(df: pd.DataFrame, cfg: dict) -> None:
    ...
    if st.button("Спрогнозировать"):
        ...
        # Крупный итог по лучшей (самой высокой вероятности) модели
        try:
            best = res_df.iloc[0]
            if pd.isna(best["Вероятность"]):
                st.warning("Итоговое решение не сформировано: для лучшей модели не удалось получить вероятность.")
            else:
                st.markdown("### Итоговое решение")
                st.metric("Максимальная вероятность обновления", f"{float(best['Вероятность'])*100:.2f}%", help=f"Модель: {best['Модель']}")
                if str(best["Решение"]).startswith("ТРЕБУЕТСЯ"):
...
"""
    ...
    # Sidebar (стабильный рендер + оформление как на скринах)
    st.sidebar.markdown('<div class="sb-title">Загрузка данных</div>', unsafe_allow_html=True)
    st.sidebar.markdown('<div class="sb-sub">Загрузите CSV с обращениями</div>', unsafe_allow_html=True)

    uploaded_file = st.sidebar.file_uploader(
        "Drag and drop file here",
        type=["csv"],
        help="Поддерживается формат CSV. Если файл не выбран, используется демонстрационный датасет.",
    )
    ...
    st.sidebar.markdown('<div class="sb-title">Раздел приложения</div>', unsafe_allow_html=True)
